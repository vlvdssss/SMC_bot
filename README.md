...
Используйте скрипт для автоматического обновления без конфликтов:
```powershell
.\update.ps1
```
...
- 📥 Загрузит обновления с GitHub
- 🔄 Восстановит ваши настройки

### Первый раз возникают конфликты Git?
Запустите один раз для исправления:
```powershell
.\fix_git_tracking.ps1
```

**Проблема:** `error: Your local changes would be overwritten by merge`  
**Решение:** 📖 [docs/GIT_PULL_FIX.md](docs/GIT_PULL_FIX.md)
